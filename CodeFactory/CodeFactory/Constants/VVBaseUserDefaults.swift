...
//  WebMaxy
//
//  Created by Mahesh Pawar on 09/09/24.
//
...
class VVBaseUserDefaults: NSObject {
    ...
    class func setSelectedCompanyObject<T : Encodable>(_ key: String, value: T) {
        let standardUserDefaults = UserDefaults.standard
        standardUserDefaults.set(try? PropertyListEncoder().encode(value), forKey: key)
        standardUserDefaults.synchronize()
    }
   /*
    class func setBrandCountryDetails<T : Encodable>(_ key: String, value: T) {
        let standardUserDefaults = UserDefaults.standard
        standardUserDefaults.set(try? PropertyListEncoder().encode(value), forKey: key)
        standardUserDefaults.synchronize()
    }
    
    class func getBrandCountryDetails<T : Decodable>(_ key: String) -> T? {
        let standardUserDefaults = UserDefaults.standard
        if let data = standardUserDefaults.value(forKey:key) as? Data {
